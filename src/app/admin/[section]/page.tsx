...
function Table({rows,columns,edit}:{rows:Row[];columns:string[];edit?:string}){return <div className="table-scroll"><table className="admin-table"><thead><tr>{columns.map(c=><th key={c}>{moneyColumns.has(c)?c.replace("_minor","").replaceAll("_"," ")+" (GHS)":c.replaceAll("_"," ")}</th>)}{edit&&<th>Action</th>}</tr></thead><tbody>{rows.length?rows.map((r,i)=><tr key={String(r.id??i)}>{columns.map(c=><td key={c}>{display(r[c],c)}</td>)}{edit&&<td><Link href={"/admin/"+edit+"?edit="+r.id}>Edit</Link></td>}</tr>):<tr><td colSpan={columns.length+1}>No records yet.</td></tr>}</tbody></table></div>}
...
orders:{title:"Orders",permission:"orders.manage",table:"orders",columns:["id","order_number","status","payment_status","total_minor","guest_email","shipping_address","created_at"]},
...
reports:{title:"Reports",permission:"reports.read",table:"orders",columns:["id","status","payment_status","total_minor","created_at"]}
};
export default async function Workspace({params,searchParams}:{params:Promise<{section:string}>;searchParams:Promise<{edit?:string;page?:string;saved?:string;error?:string}>}){
