...
import {collectPayment,saveResource,adjustStock,transitionOrder,moderateReview,assignStaff,linkCategory,uploadProductImage,saveOrderNote} from "../actions";
type Row=Record<string,unknown>;
const moneyColumns=new Set(["price_minor","compare_at_minor","cost_minor","unit_price_minor","total_minor","amount_minor","fee_minor","minimum_minor"]);
function display(v:unknown,column?:string):string{if(v===null||v===undefined)return "—";if(column&&moneyColumns.has(column)&&typeof v==="number")return money(v);return typeof v==="object"?JSON.stringify(v):String(v);}
...
export default async function Workspace({params,searchParams}:{params:Promise<{section:string}>;searchParams:Promise<{edit?:string;order?:string;page?:string;saved?:string;error?:string;range?:string;from?:string;to?:string}>}){
 ...
 {config&&(section!=="products"||q.edit)&&<><p>{config.description}</p><details className="operations-card" open={Boolean(q.edit)}><summary>{q.edit?"Edit record":"Create new"}</summary><form action={saveResource} className="operations-form"><input type="hidden" name="resource" value={section}/><input type="hidden" name="id" value={String(editing.id??"")}/>{config.fields.map(f=><label key={f.key}>{f.label}{f.key==="product_id"?<select name={f.key} defaultValue={String(editing[f.key]??"")} required><option value="">Choose product…</option>{products.map(p=><option key={String(p.id)} value={String(p.id)}>{String(p.name)}</option>)}</select>:f.key==="brand_id"?<select name={f.key} defaultValue={String(editing[f.key]??"")}><option value="">No brand</option>{brands.map(b=><option key={String(b.id)} value={String(b.id)}>{String(b.name)}</option>)}</select>:f.type==="textarea"?<textarea name={f.key} defaultValue={String(editing[f.key]??"")} required={f.required} maxLength={30000} rows={6}/>:f.type==="select"?<select name={f.key} defaultValue={String(editing[f.key]??f.options?.[0])}>{f.options?.map(o=><option key={o}>{o}</option>)}</select>:f.type==="checkbox"?<input type="checkbox" name={f.key} defaultChecked={Boolean(editing[f.key])}/>:<input name={f.key} type={f.type==="money"||f.type==="number"?"number":f.type??"text"} defaultValue={Array.isArray(editing[f.key])?(editing[f.key] as string[]).join(", "):String(f.type==="money"&&editing[f.key]!=null?Number(editing[f.key])/100:editing[f.key]??(f.type==="number"?0:""))} required={f.required} min={f.type==="number"||f.type==="money"?0:undefined} step={f.type==="money"?"0.01":f.type==="number"?1:undefined} maxLength={f.type==="number"||f.type==="money"?undefined:1000}/>}</label>)}<SubmitButton>Save record</SubmitButton>{q.edit&&<Link href={"/admin/"+section}>Cancel editing</Link>}</form></details></>}
 ...
 <Table rows={rows} columns={columns.filter(c=>c!=="id")} edit={config?section:undefined} images={section==="products"?imageMap:undefined}/>
