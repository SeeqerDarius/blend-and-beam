...
const nextConfig:NextConfig={poweredByHeader:false,images:{remotePatterns:[{protocol:"https",hostname:"gyjfofawvervookxjvmm.supabase.co",pathname:"/storage/v1/object/public/products/**"}]},experimental:{serverActions:{bodySizeLimit:"4.4mb"}},
...
{key:"Content-Security-Policy",value:"default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'none'; form-action 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https://gyjfofawvervookxjvmm.supabase.co; font-src 'self'; connect-src 'self' https://gyjfofawvervookxjvmm.supabase.co; upgrade-insecure-requests"}
